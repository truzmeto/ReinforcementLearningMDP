...
One of the implications of grid world problem that makes it interesting is rout planning. In our daily life, for instance,
we commute from home (initial state) to work(finale destination) and it is important for us to know the best rout especially
when we live in a crowded city. Also, by assigning negative reward to states associated with high crime rate blocks we can
design not just best rout but also safest rout. Traffic information can also be entered into a model to improve the rout.
Above mentioned example definitely makes grid world problem very interesting to study.
...
and policy iteration methods average reward and average number of steps converge very fast taking less than
30 iterations Fig3(a,b). In contrast, Q-learning algorithm takes a lot longer time to converge to optimal
policy. In a heatmap shown for Q-learning in Fig.2, cells (4,1) and (1,3) show the action which is opposite
...
quite complex maze problem Fig.1(right plot).  Optimal policies are illustrated in Fig4, where V-values
...
approach directly updates policies(iteratively in similar passion) in order to find optimal policy. So, then it
is not surprising to see both algorithms to result in same finale policy given adequate number of
iterations since methods have a lot in common. But, one difference we can notice is that value iteration 
...
```{r,echo=FALSE,fig.height=4.,fig.width=12,fig.cap= "Here we demonstrate convergence results for hard problem, where number of iterations are plotted against number of steps taken from any state to terminal state (a), total reward (b) and finally performance time for all three methods(c)"}
...
We have investigated practical aspects of Markov decision processes and the
reinforcement learning algorithms via value iteration, policy iteration and Q learning.
